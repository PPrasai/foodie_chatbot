...
class Zomato:
    ...
    def search_city_by_name(self, query):
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        r = (requests.get(base_url + "cities?q=" + query, headers = headers).content).decode("utf-8")
        return r

    def search_cuisines_in_city(self, city_id):
        headers = {
            'Accpet': 'application/json',
            'user-key': self.user_key
        }

        result = (requests.get(base_url + 'cuisines?city_id='+ str(city_id), headers = headers).content).decode('utf-8')
        return result

    def search_restaurant_in_city(self, city_id, limit, offset):
        headers = {
            'Accept': 'application/json',
            'user-key': self.user_key
        }

        url = base_url + 'search?entity_id='+ str(city_id) +'&entity_type=city&sort=rating'
    ...
    def search_restaurant_in_city_with_cuisine(self, city_id, cuisine, limit, offset):
        headers = {
            'Accept': 'application/json',
            'user-key': self.user_key
        }

        url = base_url + 'search?entity_id='+ str(city_id) +'&entity_type=city'
        ...
        url = url + '&cuisines='+ str(cuisine)+ '&sort=rating'
